...
void MX_I2C2_Init(void)
{
  ...
  */
  ...
  /* I2C2 interrupt Init */
  NVIC_SetPriority(I2C2_EV_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(),10, 0));
  NVIC_EnableIRQ(I2C2_EV_IRQn);
  NVIC_SetPriority(I2C2_ER_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(),10, 0));
...
ErrorStatus my_I2C_At24c256_Write (I2C_TypeDef * I2Cx , uint8_t * p_data , uint32_t len , uint8_t slave_addr , uint16_t mem_addr)
{
  ...
  //先发送低地址
  LL_I2C_TransmitData8 (I2Cx , mem_addr);

  while (!LL_I2C_IsActiveFlag_TXE (I2Cx));

  //再发送高地址
  LL_I2C_TransmitData8 (I2Cx , mem_addr >> 8);
...
ErrorStatus my_I2C_At24c256_Read (I2C_TypeDef * I2Cx , uint8_t * p_data , uint32_t len , uint8_t slave_addr , uint16_t mem_addr)
{
  ...
  for (uint16_t i = 0; i < len; i++)
  {
    ...
    p_data[i] = LL_I2C_ReceiveData8 (I2Cx);  //接收数据
  }
